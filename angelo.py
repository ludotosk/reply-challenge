...
output = open("outputFile-angelo-05.txt", "w")
...
def find_best_fit(lista_sfidabili, Tn):
    ...
    for sfidabile in lista_sfidabili:
        ...
        upperParameter = min(len(sfidabile.punti_int), (turn-Tn))-1

        # score = ((sfidabile.stamina_recuperata - sfidabile.stamina_consumata) / sfidabile.turni_recupero) * sum(sfidabile.punti_int[:upperParameter])
        score = ((sfidabile.stamina_recuperata - sfidabile.stamina_consumata) / sfidabile.turni_recupero) * sum(sfidabile.punti_int[:upperParameter])
        if(score > max_score):
            bestDemon = sfidabile
...
def chooseBestDemon(stamina_Corrente,t):
    sfidabili = find_sfidabili()

    if(sfidabili == []) :
        return None
    
    bestDemon = find_best_fit(sfidabili,t)
...
for t in range(turn):
    ...
    demon = chooseBestDemon(stamina_Corrente,t)
